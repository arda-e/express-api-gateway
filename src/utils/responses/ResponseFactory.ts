...
export class ResponseFactory {
    ...
    static createResource<T extends BaseModel>(item: T, baseUrl: string): [number, Resource<T>] {
        const links = new ResourceLinks(
            new Link(`${baseUrl}/${item.id}`),
            {
                update: new Link(`${baseUrl}/${item.id}`, 'PUT'),
                delete: new Link(`${baseUrl}/${item.id}`, 'DELETE')
            }
        );
        return [StatusCodes.CREATED, new Resource(item.id, item.created_at, item.updated_at, links, item)];
    }
    /**
     ...
     */
    static getSingleResource<T extends BaseModel<T>>(resource: T, baseUrl: string): [number, GetResponse<T>] {
     ...
     */
    static createUpdateResponse<T extends Resource<T>>(updatedResource: T, baseUrl: string): [number,UpdateResponse<T>] {
